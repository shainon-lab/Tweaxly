// Reusable demo seeder. Lifted from scripts/seed-shainon-agency.ts
// so the onboarding 'Explore Demo Business' button can seed the
// caller's current workspace with a believable digital-agency
// dataset (Jan 2023 → May 2026, 7 employees, ~1,400 transactions).
//
// Idempotent: wipes the workspace's existing data before seeding.
...
type EmployeeDef = {
  ...
  grossMonthlySalary: number; start: { y: number; m: number };
};
...
// Wipe existing data for the target business in FK-safe order. The
// Business itself + its memberships stay intact.
async function wipeBusinessData(businessId: string) {
  await prisma.consultationMessage.deleteMany({ where: { consultation: { businessId } } });
  await prisma.consultation.deleteMany({ where: { businessId } });
  await prisma.recommendation.deleteMany({ where: { businessId } });
  await prisma.transaction.deleteMany({ where: { businessId } });
  await prisma.manualEntry.deleteMany({ where: { businessId } });
  await prisma.uploadBatch.deleteMany({ where: { businessId } });
  await prisma.duplicateGroup.deleteMany({ where: { businessId } });
  await prisma.notificationRule.deleteMany({ where: { businessId } });
  await prisma.forecastAssumption.deleteMany({ where: { businessId } });
  await prisma.forecastItem.deleteMany({ where: { businessId } });
  await prisma.mutedSignal.deleteMany({ where: { businessId } });
  await prisma.employeeEvent.deleteMany({ where: { businessId } });
  await prisma.employee.deleteMany({ where: { businessId } });
  await prisma.categorizationRule.deleteMany({ where: { businessId } });
  await prisma.mappingTemplate.deleteMany({ where: { businessId } });
  await prisma.vendor.deleteMany({ where: { businessId } });
  await prisma.category.deleteMany({ where: { businessId } });
}

export async function seedDemoBusiness(businessId: string): Promise<{
  transactions: number;
}> {
  ...
  const catRows = await prisma.$transaction(
    CATEGORIES.map((c) => prisma.category.create({
      data: { businessId, name: c.name, kind: c.kind, isOneTime: c.isOneTime },
    }))
  );
  ...
  for (const v of VENDORS) {
    await prisma.vendor.create({
      data: { businessId, name: v.name, categoryId: catId(v.categoryName), isOneTime: !!v.isOneTime },
    });
  }
  ...
  for (const e of EMPLOYEES) {
    await prisma.employee.create({
      data: {
        businessId, name: e.name, role: e.role, department: e.department,
        employmentType: "employee", grossMonthlySalary: e.grossMonthlySalary,
        employerCostMultiplier: 1.25, startDate: date(e.start.y, e.start.m, 1),
        status: "active",
      },
    });
  }
  ...
  for (const m of months) {
    ...
    if (total > 0) {
      // Retainer block (~50% of revenue split across 3 clients)
      const retainerPortion = total * (0.45 + rand() * 0.20);
      const retainerClients = ["Acme Corp", "BlueSky Ventures", "Northstar SaaS", "Vivid Retail"];
      const active = retainerClients.slice(0, rInt(3, 4));
      const weights = active.map(() => 0.8 + rand() * 0.4);
      const wSum = weights.reduce((a, b) => a + b, 0);
      active.forEach((client, i) => {
        const amount = Math.round((retainerPortion * weights[i]) / wSum);
        if (amount > 0) {
          allTxns.push({
            date: date(m.y, m.m, rInt(1, 7)), amount, vendor: client,
            categoryName: "Retainer Revenue", description: `Monthly retainer · ${client}`,
            type: "income", source: "bank", ym: monthLabel,
          });
        }
      });
      // Project work
      const projClients = ["Apex Labs", "Greenleaf Co", "Sundial Media", "Helio Industries", "Cobalt Health"];
      const projCount = rInt(1, 3);
      const projPortion = total - allTxns.filter((x) => x.ym === monthLabel).reduce((a, t) => a + t.amount, 0);
      if (projPortion > 0 && projCount > 0) {
        const pw = Array.from({ length: projCount }, () => 0.6 + rand() * 0.8);
        const pwSum = pw.reduce((a, b) => a + b, 0);
        for (let i = 0; i < projCount; i++) {
          const amount = Math.round((projPortion * pw[i]) / pwSum);
          if (amount > 0) {
            allTxns.push({
              date: date(m.y, m.m, rInt(8, 25)), amount, vendor: pick(projClients),
              categoryName: "Project Revenue", description: `Project milestone payment`,
              type: "income", source: "bank", ym: monthLabel,
            });
          }
        }
    ...
    for (const e of EMPLOYEES) {
      ...
      allTxns.push({
        date: date(m.y, m.m, last - 1), amount: -loaded, vendor: e.name,
        categoryName: "Payroll", description: `Payroll · ${e.name} (${e.role})`,
        type: "payroll", source: "bank", ym: monthLabel,
      });
    }
    ...
    for (const s of SAAS_RECURRING) {
      ...
      allTxns.push({
        date: date(m.y, m.m, rInt(1, 5)), amount, vendor: s.vendor,
        categoryName: "Software & SaaS", description: `${s.vendor} subscription`,
        type: "expense", source: "credit_card", ym: monthLabel,
      });
    }

    // Rent + utilities from Jun 2023
    if (monthLabel >= "2023-06") {
      allTxns.push({
        date: date(m.y, m.m, 1), amount: -3_500, vendor: "Mission Street Lease",
        categoryName: "Office Rent", description: "Monthly office lease",
        type: "expense", source: "bank", ym: monthLabel,
      });
      allTxns.push({
        date: date(m.y, m.m, rInt(3, 8)), amount: -Math.round(r(180, 320)),
        vendor: "PG&E", categoryName: "Utilities", description: "Electricity",
        type: "expense", source: "bank", ym: monthLabel,
      });
      allTxns.push({
        date: date(m.y, m.m, rInt(3, 8)), amount: -Math.round(r(120, 180)),
        vendor: "Comcast Business", categoryName: "Utilities", description: "Business internet",
        type: "expense", source: "bank", ym: monthLabel,
      });
    }

    // Marketing (phase-scaled, Q4 boost)
    let mktBudget = 1_500;
    if (monthLabel >= "2024-01" && monthLabel < "2024-10") mktBudget = 800;
    ...
    for (const v of ["Google Ads", "Meta Ads", "LinkedIn Ads"]) {
      allTxns.push({
        date: date(m.y, m.m, rInt(2, 28)), amount: -Math.round(mktBudget * r(0.25, 0.5)),
        vendor: v, categoryName: "Marketing & Ads", description: `${v} campaigns`,
        type: "expense", source: "credit_card", ym: monthLabel,
      });
    }
    if (monthLabel >= "2024-03") {
      allTxns.push({
        date: date(m.y, m.m, rInt(1, 5)), amount: -Math.round(r(680, 850)),
        vendor: "HubSpot", categoryName: "Marketing & Ads", description: "HubSpot Pro subscription",
        type: "expense", source: "credit_card", ym: monthLabel,
      });
    }
    ...
    if (rand() < contractorChance) {
      allTxns.push({
        date: date(m.y, m.m, rInt(5, 25)), amount: -Math.round(r(1_500, 6_000)),
        vendor: pick(["Upwork", "Toptal", "Freelance Hub"]), categoryName: "Contractors",
        description: "Freelance overflow work", type: "expense", source: "bank", ym: monthLabel,
      });
    }
    ...
    if (m.m === 1 || m.m === 4 || m.m === 7 || m.m === 10) {
      allTxns.push({
        date: date(m.y, m.m, rInt(10, 25)), amount: -Math.round(r(900, 1_800)),
        vendor: "Sterling Accountancy", categoryName: "Professional Services",
        description: "Quarterly bookkeeping", type: "expense", source: "bank", ym: monthLabel,
      });
    }
    if (rand() < 0.15) {
      allTxns.push({
        date: date(m.y, m.m, rInt(5, 25)), amount: -Math.round(r(500, 2_500)),
        vendor: "Vega Legal", categoryName: "Professional Services",
        description: "Legal review", type: "expense", source: "bank", ym: monthLabel,
      });
    }

    // Insurance, supplies, fees, travel
    if (monthLabel >= "2023-04") {
      allTxns.push({
        date: date(m.y, m.m, 5), amount: -Math.round(r(420, 580)),
        vendor: "Hartford Insurance", categoryName: "Insurance",
        description: "Business liability + benefits", type: "expense", source: "bank", ym: monthLabel,
      });
    }
    allTxns.push({
      date: date(m.y, m.m, rInt(5, 25)), amount: -Math.round(r(150, 600)),
      vendor: pick(["Amazon Business", "Costco Business"]), categoryName: "Office Supplies",
      description: "Office supplies", type: "expense", source: "credit_card", ym: monthLabel,
    });
    if (rand() < 0.6) {
      const meals = rInt(2, 6);
      for (let i = 0; i < meals; i++) {
        allTxns.push({
          date: date(m.y, m.m, rInt(2, 28)), amount: -Math.round(r(35, 180)),
          vendor: pick(["Uber", "DoorDash"]), categoryName: "Travel & Meals",
          description: "Team meals / transit", type: "expense", source: "credit_card", ym: monthLabel,
        });
      }
    }
    if (rand() < 0.18 && monthLabel >= "2023-09") {
      allTxns.push({
        date: date(m.y, m.m, rInt(5, 25)), amount: -Math.round(r(380, 900)),
        vendor: "Delta", categoryName: "Travel & Meals", description: "Client visit flight",
        type: "expense", source: "credit_card", ym: monthLabel,
      });
    }
    if (rand() < 0.08) {
      allTxns.push({
        date: date(m.y, m.m, rInt(5, 25)), amount: -Math.round(r(1_200, 3_200)),
        vendor: pick(["Apple", "Dell"]), categoryName: "Equipment",
        description: "New laptop / monitor", type: "expense", source: "credit_card", ym: monthLabel,
      });
    }
    allTxns.push({
      date: date(m.y, m.m, last), amount: -Math.round(r(60, 220)),
      vendor: "Stripe", categoryName: "Bank Fees", description: "Payment processing fees",
      type: "fee", source: "bank", ym: monthLabel,
    });
    allTxns.push({
      date: date(m.y, m.m, last), amount: -Math.round(r(15, 55)),
      vendor: "Mercury Bank", categoryName: "Bank Fees", description: "Bank service fees",
      type: "fee", source: "bank", ym: monthLabel,
    });
  }
  ...
  for (let i = 0; i < allTxns.length; i += BATCH) {
    ...
    await prisma.transaction.createMany({
      data: slice.map((t) => ({
        businessId, source: t.source, transactionDate: t.date, accountingMonth: t.ym,
        amount: t.amount, currency: "USD", type: t.type, categoryId: catId(t.categoryName),
        vendor: t.vendor, description: t.description,
      })),
    });
  }

  return { transactions: allTxns.length };
}
