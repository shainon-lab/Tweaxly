...
// panes: profile-and-branding, categories-and-rules, integrations, and
// account (auth + billing).
type SettingsTab = "profile" | "categories" | "integration" | "account";
type AccountSubTab =
  | "billing"
  | "payment"
  | "password"
...
export default function SettingsClient({
  ...
  const [accountTab, setAccountTab] = useState<AccountSubTab>("billing");
...
function AccountTab({
  ...
  user: { email: string; createdAt: string };
  accountTab: AccountSubTab;
  setAccountTab: (t: AccountSubTab) => void;
}) {
  ...
  return (
    <>
      ...
      {accountTab === "billing"       ? <BillingPane /> : null}
      {accountTab === "payment"       ? <PaymentMethodsPane /> : null}
      {accountTab === "password"      ? <PasswordPane user={user} /> : null}
      {accountTab === "access_logs"   ? <AccessLogsPane /> : null}
      {accountTab === "close_account" ? <CloseAccountPane /> : null}
    </>
  );
}
  ...
  return (
    <div className="card">
      ...
      <div className="text-sm text-slate-400 mb-4 leading-relaxed">
        Manage your plan, view invoices, and switch products from here.
      </div>
      ...
      <div className="text-xs text-slate-500">
        Paid plans, invoices, and per-product billing land here once we exit preview. We&apos;ll email you before any change applies.
      </div>
    </div>
  );
}
